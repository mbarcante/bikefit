...
import { IBike } from "../../../interfaces"; // Sua interface IBike
    ...
    <Card className="mb-3">
      <Card.Header className="d-flex justify-content-between align-items-center">
        ...
        <Button variant="success" size="sm">
          <i className="bi bi-plus-circle"></i> Adicionar Bicicleta
        </Button>
      </Card.Header>
      <Card.Body>
        ...
        {!loading && !error && bikes.length === 0 && (
          <p>Nenhuma bicicleta encontrada para este cliente.</p>
        )}
        {!loading && !error && bikes.length > 0 && (
              ...
              <Accordion.Item eventKey={String(index)} key={bike.id}>
                <Accordion.Header>
                  <h6 className="mb-0">
                    ...
                    {bike.year ? bike.year + " - " : ""}ID: {bike.id})
                  </h6>
                </Accordion.Header>
                <Accordion.Body>
                  <Row>
                  ...
                  <Row>
                    <Col md={6}>
                      <p className="mb-1 text-muted">
                        Cliente ID: {bike.clientId}
                      </p>
                    </Col>
                    <Col md={6}>
                      <p className="mb-1 text-muted">
                        Cadastro: {/* CORREÇÃO AQUI: Use bike.DtCadastro */}
                        {bike.DtCadastro
                          ? new Date(bike.DtCadastro).toLocaleDateString()
                          : "N/A"}
                      </p>
                    </Col>
                  </Row>
